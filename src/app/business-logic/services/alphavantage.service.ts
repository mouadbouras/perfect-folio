...
export class AlphavantageService {
  ...
  public searchSymbol(symbol: string): Observable<AlphaVantageSecurity[]> {
    return this._http
      .get(`${this.url}${this.searchFunction}&keywords=${symbol}${this.apiKey}`)
      .pipe(
        filter((response: any) => !response['Note']),
        map((response: any) =>
          this.responseToSecurities(response['bestMatches'])
        )
      );
  }
  ...
  public getQuoteForSymbol(symbol: string): Observable<AlphaVantageQuote> {
    return this._http
      .get(`${this.url}${this.quoteFunction}&symbol=${symbol}${this.apiKey}`)
      .pipe(
        filter((response: any) => !response['Note']),
  ...
  public getCurrencyExchange(
    ...
    return this._http
      .get(
        `${this.url}${this.exchangeFunction}&from_currency=${fromCurrency}&to_currency=${toCurrency}${this.apiKey}`
      )
      .pipe(
        filter((response: any) => !response['Note']),
        map(
